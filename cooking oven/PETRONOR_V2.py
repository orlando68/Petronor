...
def oil_whip(df_in):
    ...
    for i in range (n_traces):
        A = (df_in.iloc[i]['E 1/2'] >= E1 and df_in.iloc[i]['SPAN 1/2'] >= 4)  and  ((df_in.iloc[i]['E 5/2'] >= E1) and df_in.iloc[i]['SPAN 2.5'] >= 4)
        ...
        print(A,B,C)
                                             ...
                                             #-----------green-----------------
                                             # 2H SPAN at 0.5 = 0 and 2H SPAN at 2.5 = 0                                        
        ...
        if A == False and ( (B and C) == False ):  
            df_in.loc[df_in.index[i],'Oil Whip Failure'] = 'Green'            
                                             ...
                                             # 2H SPAN at 0.5 > 0
                                             # 2H SPAN at 2.5 > 0
                                             # 2H SPAN at 0.5 >2% 1.0x
                                             # 2H SPAN at 2.5 >2% 1.0x
        if A ^ ((B ^ C)) :  
...
def Loose_Bedplate(df_in):
    ...
    for i in range (n_traces):
        ...
        A = 2.5 * np.sqrt(2) > df_in.iloc[i]['E 1.0'] > 0.15 * np.sqrt(2)
...
def df_Harmonics(df_FFT,df_time,fs):
    ...
    freq_elements = ['f 1/5x' ,'f 1/4x' ,'f 1/3x',
                     'f 2/3x' ,'f 4/3x' ,'f 5/3x','f 8/3x',
                     ...
                     'f 11/1x','f 12/1x','f 13/1x',
                     'f 23/1x','f 24/1x','f 25/1x',
                     
                     'f 2nd Highest',
                     'B 0.38x-0.48x O. Whirl',
                     'B 12-24 Flow T.',
                     'B 0.38x-0.5x Surge E.',
                     'B 12000-24000 Surge E.']
    
    
    for k in freq_elements:
        word = k[2:len(k)+1]
        columnas.append('Pk(f) Hz '+word)
        columnas.append('Pk(i) '+word)
        columnas.append('Pk(P) mm/s '+word)
        columnas.append('Pk(E) mm/s '+word)
        columnas.append('PK(Span) Hz '+word)
...
#            word =str(int(3*k)) +'/3'  
    ...
    for k in np.arange(3,5):
        ...
        columnas.append('Span 1/'+str(k))

    resto =      ['i Oil Whirl'      ,'P Oil Whirl'      ,'E Oil Whirl'      ,'f Oil Whirl'      ,'Span Oil Whirl'      ,
                  'i Flow T.'        ,'P Flow T.'        ,'E Flow T.'        ,'f Flow T.'        ,'Span Flow T.'        ,
                  'i Surge E.'       ,'P Surge E.'       ,'E Surge E.'       ,'f Surge E.'       ,'Span Surge E.'       ,
                  'i Surge E. 12/20k','P Surge E. 12/20k','E Surge E. 12/20k','f Surge E. 12/20k','Span Surge E. 12/20k',
                  'i 2nd Highest'    ,'P 2nd Highest'    ,'E 2nd Highest'    ,'f 2nd Highest'    ,'Span 2nd Highest'    ,
                  'T. RMS(t)'        ,'T. RMS(f)']         
    ...
    for medida in range(n_traces):
        ...
        if 24<f[n_maxi_C]<25.5:
            ...
            df_harm.iloc[medida]['Span 2nd Highest'] = properties["widths"][i_L2]
            ...
            for counter,k in enumerate(indexes) :   
                ...
                for h in df_harm.columns:
                    #print(h,f[0])
                    if h[0] == 'f':
                        print('>>>>>>>>>>>>>>>>>>>>>>>>',h.split(' ')[1].split('/'))
                ...
                if  f_1x*0.38   <= f[k] <= f_1x*0.48:
                    ...
                    if piko > df_harm.iloc[medida]['E Oil Whirl']:
                        ...
                        df_harm.iloc[medida]['Span Oil Whirl'] = f[int(properties["widths"][counter]) ]
                ...
                if  12   <= f[k] <= 24:
                    ...
                    if piko > df_harm.iloc[medida]['E Flow T.']:
                        ...
                        df_harm.iloc[medida]['Span Flow T.'] = f[int(properties["widths"][counter]) ]                    
                #---------------------------------------------1/n sub-harmonics------------------       
                for w in subpeaks:
                    if 1/w -delta  <= f[k]/f_1x <= 1/w +delta:
                        ...
                        if piko > df_harm.iloc[medida]['E '+word]:
                            ...
                            df_harm.iloc[medida]['Span '+word] = f[int(properties["widths"][counter]) ]
                #---------------------------------------------Natural-harmonics------------------            
                for w in peaks:                                        
                ...
                if  f_1x*0.38   <= f[k] <= f_1x*0.5:
                    ...
                    if k!=df_harm.iloc[medida]['i Oil Whirl']  and  k!=df_harm.iloc[medida]['i Flow T.']:
                        ...
                        if piko > df_harm.iloc[medida]['E Surge E.']:
                            ...
                            df_harm.iloc[medida]['Span Surge E.'] = f[int(properties["widths"][counter]) ]                
                    else:
...
#--------------------------------------------------------------------------------SURGE EFFECT 12 a 20kHz
                if  12000   <= f[k] <= 20000:
                    ...
                    if piko > df_harm.iloc[medida]['E Surge 12/20k']:
                        ...
                        df_harm.iloc[medida]['Span Surge 12/20k'] = f[int(properties["widths"][counter]) ]
...
if load_DataBase:
    ...
    print('Leyendo de un fichero pickle')
