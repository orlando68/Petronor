    ...
    for root, dirs, files in os.walk(rootdir):
        ...
        for filename in files:
            ...
            if res.AssetId.values[0] == assetId and res.MeasurePointId.values[0] == MeasurePointId:
                ...
                print(datetime.datetime.fromtimestamp(segundos),MeasurePointId)
...
def load_vibrationDataII(rootdir, assetId,MeasurePointId,f_init,f_end):
    
    data     = []
    date     = []
    
    f_init_s = time.mktime(f_init.timetuple())
    f_end_s  = time.mktime(f_end.timetuple())
    print (f_init_s,f_end_s)
    format   = "%Y-%m-%dT%H:%M:%S"
    ...
    for root, dirs, files in os.walk(rootdir):
        ...
        for filename in files:
            ...
            fecha = res.ServerTimeStamp.values[0]
            datetime_obj = datetime.datetime.strptime(fecha[0:19],format)
            segundos = time.mktime(datetime_obj.timetuple())
            
            if (res.AssetId.values[0] == assetId) and (res.MeasurePointId.values[0] == MeasurePointId) and (f_init_s<=segundos<=f_end_s):
...
#                datetime_obj = datetime.datetime.strptime(fecha[0:19],format)
#                segundos = time.mktime(datetime_obj.timetuple())
                print(datetime.datetime.fromtimestamp(segundos))
                date.append(segundos)
...
def df_unbalance(df_in):
    ...
    for i in range (n_traces):
        #print(df_in.iloc[i].values[1:8])
                                            # max armonicos = 1.0x
        f_max1 = np.max( df_in.iloc[i].values[1:8] )
                                            # max del resto de armónicos
        s_max1 = np.max( df_in.iloc[i].values[2:8] )
...
def oil_whirl(df_in):
    ...
    for i in range (n_traces):
        ...
        if df_in.iloc[i]['0.38-0.48'] == 0:  
            df_in.loc[df_in.index[i],'Oil Whirl Failure'] = 'Green'
                                            #-----------yellow-----------------
                                            # Detected Peak in '0.38-0.48'
                                            #         but
                                            # Peak in '0.38-0.48' < 2% 1.0x
        if df_in.iloc[i]['0.38-0.48'] != 0 and df_in.iloc[i]['0.38-0.48'] < 0.02 * df_in.iloc[i]['1.0']:
...
def df_Harmonics(df_FFT,df_time,fs,fichero):
    ...
    sheet1.write( 0,1,'0.5')
    sheet1.write( 0,2,'1.0') 
    sheet1.write( 0,3,'1.5')
    sheet1.write( 0,4,'2.0')
    sheet1.write( 0,5,'2.5')
    sheet1.write( 0,6,'3.0')
    sheet1.write( 0,7,'3.5')
    sheet1.write( 0,8,'4.0')
    l          = df_FFT.shape[1]
    ...
    columnas   = ['0.38-0.48','f 0.38-0.48',
    ...
    for medida in range(n_traces):
        df_out     = pd.DataFrame(data    = np.ones((3,8))-2,
                                  index   = ['Hz','N_freq','N_Amp'],
        ...
        TRH        = np.std(sptrm_C[0:l_mitad])/2
        indexes    = detect_peaks(sptrm_C[0:l_mitad], mph = TRH , mpd = 5*l/fs)
        ...
        Del_Hz_M = 4
                                            #----------------------------------
        for k in indexes:
            #print ( np.sqrt(np.sum  (sptrm_C [int(f_1x*0.38*l/fs) : int(f_1x*0.48*l/fs)]**2)  ) )
            
                                            #----OIL Whirl--------peak in BW---
            if  np.max( sptrm_C [int(f_1x*0.38*l/fs) : int(f_1x*0.48*l/fs)] )    >= TRH :
                ...
                df_harm.iloc[medida]['0.38-0.48']   = sptrm_C [i_piko]
                df_harm.iloc[medida]['f 0.38-0.48'] = f[i_piko]
            ...
            if  f_1x - Del_Hz_P     <= f[k] <= f_1x     + Del_Hz_P:
                ...
                df_harm.iloc[medida]['1.0'] = piko #/ Max_value
            ...
            if  f_1x*2 - Del_Hz_P   <= f[k] <= f_1x*2   + Del_Hz_P:
                piko = np.sqrt(np.sum( sptrm_C[k-D_Hz_Peak  : k-D_Hz_Peak +int_wind_P]**2 ))
            ...
            if  f_1x*3 - Del_Hz_P   <= f[k] <= f_1x*3   + Del_Hz_P:
                piko = np.sqrt(np.sum( sptrm_C[k-D_Hz_Peak  : k-D_Hz_Peak +int_wind_P]**2 ))
            ...
            if  f_1x*7/2 - Del_Hz_M <= f[k] <= f_1x*7/2 + Del_Hz_M:
                ...
                #print()
            if  f_1x*4- Del_Hz_P    <= f[k] <= f_1x*4   + Del_Hz_P:
                piko = np.sqrt(np.sum( sptrm_C[k-D_Hz_Peak  : k-D_Hz_Peak +int_wind_P]**2 ))
        ...
        instante = df_SPEED.index[medida]
        #instante = df_harm.index[medida]
        #print(instante)
        write_to_excel(sheet1,instante,df_out,medida)
...
def plot_waterfall2(df_in,fs,fmin,fmax):
    ...
    second_plot = plt.axes([.02, .75, .2, .2], facecolor='w')
...
#df_accel,l_devices = load_vibrationData(path,maquina,localizacion);df_accel.to_pickle('accel_'+maquina+'_'+localizacion+'_'+month+'_'+day+'.pkl')  #------------en G´s-
df_accel           = pd.read_pickle('accel_'+maquina+'_'+localizacion+'_'+month+'_'+day+'.pkl') 
...
n_file_SPEED       = 'SPEED_'+maquina+'_'+localizacion+'_'+month+'_' + day
df_accel.to_pickle( n_file_SPEED + '.pkl')
#df_SPEED           = pd.read_pickle('SPEED_'+maquina+'_'+localizacion+'_'+month+'_'+day+'.pkl') 


find_closest(datetime.datetime(2018, 10, 1, 0, 0))
...
"""
for i in range(n_traces):
    SPTRM,f,df=clearance(df_speed_f.iloc[i].values,fs,'Velocidad con HPF','RMS' ,Detection = 'RMS')
    instante = df_SPEED.index[i]
    write_to_excel(sheet1,instante,df,i)
"""
...
df_plot = df_SPEED
traces = df_plot.index.values
traces = traces-traces[0]
array_n1max = np.zeros(n_traces) 
array_1max = np.zeros(n_traces)
radio = 2 
int_window = 2*radio+1
array_1max_3p = np.zeros(n_traces) 
plt.figure()
for counter,indice in enumerate(df_plot.index):
    curva               = cte * df_plot.iloc[counter].values
    plt.plot(curva[np.argmax(curva)-radio : np.argmax(curva)-radio+int_window])
    array_n1max[counter]= np.argmax(curva)
    array_1max[counter]= np.max(curva)
    array_1max_3p[counter]= np.sum( curva[np.argmax(curva)-radio : np.argmax(curva)-radio+int_window] )
...
plt.figure()
plt.plot(array_1max)
plt.plot(array_1max_3p)
plt.show()

plt.figure()
n, bins, patches = plt.hist(array_1max, 100,density=True, facecolor='g', alpha=0.75)
plt.show()

#plt.figure()
n, bins, patches = plt.hist(array_1max_3p, 100,density=True, facecolor='r', alpha=0.75)
plt.show()
"""
"""
#----------------------------------------------------------------
