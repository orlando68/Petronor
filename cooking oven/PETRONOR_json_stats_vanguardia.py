...
def df_Harmonics(df_FFT,df_time,fs,fichero):
    ...
    columnas = []
    peaks    = []
    for k in np.arange(0.5,20,0.5):
        peaks.append(k) 
        columnas.append('i '+str(np.float(k)))
        columnas.append(str(np.float(k)))
    ...
    for k in np.arange(3,6):
        ...
        columnas.append('1/'+str(k))
        columnas.append('f 1/'+str(k))
        columnas.append('BW 1/'+str(k))
    """
    trozo =      ['i 1/4'        ,'1/4'        ,'f 1/4'        ,'BW 1/4'        ,
                  'i 1/3'        ,'1/3'        ,'f 1/3'        ,'BW 1/3'        ,
                  'i Oil Whirl'  ,'Oil Whirl'  ,'f Oil Whirl'  ,'BW Oil Whirl'  ,
        ...
        for counter,k in enumerate(indexes) :       
            ...
            if  12   <= f[k] <= 24:
                ...
                if piko > df_harm.iloc[medida]['Flow T.']:
                    ...
                    df_harm.iloc[medida]['BW Flow T.'] = f[int(properties["widths"][counter]) ]                    
                             #-----------------------------------------------------------1/3x-----------                                                                               
            if  1/3-delta   <= f[k]/f_1x <= 1/3+delta  :
                piko                          = np.sqrt(np.sum( sptrm_C[ int(properties["left_ips"][counter]) : int(properties["right_ips"][counter]) ]**2 ))
                print('>>>>>>    1/3    >>>>>>>>>>')               
                if piko > df_harm.iloc[medida]['1/3']:
                    ...
                    df_harm.iloc[medida]['BW 1/3'] = f[int(properties["widths"][counter]) ]
                             #-----------------------------------------------------------1/4x----------- 
            if  1/4-delta   <= f[k]/f_1x <= 1/4+delta  :
                piko                          = np.sqrt(np.sum( sptrm_C[ int(properties["left_ips"][counter]) : int(properties["right_ips"][counter]) ]**2 ))
                print('>>>>>>    1/4    >>>>>>>>>>')
            ...
            for w in peaks:
                if w -delta  <= f[k]/f_1x <= w +delta:
                    piko                          = np.sqrt(np.sum( sptrm_C[ int(properties["left_ips"][counter]) : int(properties["right_ips"][counter]) ]**2 ))
                    print ('XXXXXXXXXXXXXXXXXXXXXXXXX',w)
                    word = str(np.float(w))
                    if piko > df_harm.iloc[medida][word]:
                        df_harm.iloc[medida]['i '+word ] = k
                        df_harm.iloc[medida][word   ] = piko #/ Max_value
