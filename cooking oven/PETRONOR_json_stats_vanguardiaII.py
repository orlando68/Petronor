...
def df_clearance(df_in):
    ...
    for i in range (n_traces):
        ...
        v_0_5x = df_in.iloc[i]['E 0.5']
        v_1_5x = df_in.iloc[i]['E 1.5']
        v_2_5x = df_in.iloc[i]['E 2.5']
...
def oil_whip(df_in):
    ...
    for i in range (n_traces):
        A = (df_in.iloc[i]['E 0.5'] >= E1 and df_in.iloc[i]['BW 0.5'] >= 4)  and  ((df_in.iloc[i]['E 2.5'] >= E1) and df_in.iloc[i]['BW 2.5'] >= 4)
        B = df_in.iloc[i]['E 0.5' ] > 0.02 *  df_in.iloc[i]['E 1.0']
        C = df_in.iloc[i]['E 2.5' ] > 0.02 *  df_in.iloc[i]['E 1.0']        
        ...
        if A == False and ( (B and C) == False ):  
            df_in.loc[df_in.index[i],'E Oil Whip Failure'] = 'Green'            
                                             #---------yellow------------------
                                             # 2H BW at 0.5 > 0
                                             # 2H BW at 2.5 > 0
                                             # 2H BW at 0.5 >2% 1.0x
        ...
        if A == True or ((B or C) == True) :  
            df_in.loc[df_in.index[i],'E Oil Whip Failure'] = 'Yellow'           
                                             #-----------red-------------------    
                                             #     2H BW at 0.5 >2% 1.0x
                                             #           AND
                                             #     2H BW at 2.5 >2% 1.0x
        if A and B and C:  
            df_in.loc[df_in.index[i],'E Oil Whip Failure'] = 'Red'    
    return df_in
#------------------------------------------------------------------------------
...
def Blade_faults(df_in):
    ...
    for i in range (n_traces):
        none_list.append('None')        
    df_in['Blade Faults'] = none_list
    
    for i in range (n_traces):
        A = df_in.iloc[i]['E 1BPF'] > E1
        B = df_in.iloc[i]['E 1BPF'] > E1 and df_in.iloc[i]['E 2BPF'] > E1
        C = df_in.iloc[i]['E 1BPF'] > E1 and (df_in.iloc[i]['E 1BPF-1.0'] > E1 or df_in.iloc[i]['E 1BPF+1.0'] > E1)
        D = C and df_in.iloc[i]['E 2BPF'] > E1
        E = C and df_in.iloc[i]['E 2BPF'] > E1 and (df_in.iloc[i]['E 2BPF-1.0'] > E1 or df_in.iloc[i]['E 2BPF+1.0'] > E1)
        F = df_in.iloc[i]['E 1BPF'] < E1 and df_in.iloc[i]['E 2BPF'] < E1
        #print('Blade Faults         ',A,B,C,D,E,F)
                                             #  Tabla de verdad progresiva
                                             #  puede empezar siendo verde,
                                             #  acabar siendo rojo
        if A or B or F:  
            df_in.loc[df_in.index[i],'Blade Faults'] = 'Green'            
        if C or D:  
            df_in.loc[df_in.index[i],'Blade Faults'] = 'Yellow'        
        if E:  
            df_in.loc[df_in.index[i],'Blade Faults'] = 'Red'
...
def flow_turbulence(df_in):
    ...
    for i in range (n_traces):
        ...
        if A:
            df_in.loc[df_in.index[i],'Flow Turbulence'] = 'Green'
        if B:
            df_in.loc[df_in.index[i],'Flow Turbulence'] = 'Yellow'
        if C:
            df_in.loc[df_in.index[i],'Flow Turbulence'] = 'Red'
...
def Plain_bearing_block_looseness(df_in):
    ...
    for i in range (n_traces):
        ...
        B = df_in.iloc[i]['E 0.5'] > E1 and df_in.iloc[i]['E 1/3'] > E1 and df_in.iloc[i]['E 1/4'] > E1
...
def df_Harmonics(df_FFT,df_time,fs):
    ...
    for k in np.arange(0.5,5.5,0.5):
        peaks.append(k) 
        columnas.append('i '+str(np.float(k)))
        columnas.append('P '+str(np.float(k)))
        columnas.append('E '+str(np.float(k)))
        columnas.append('f '+str(np.float(k)))
        columnas.append('BW '+str(np.float(k)))
    ...
    resto =      ['i Oil Whirl'  ,'P Oil Whirl'  ,'E Oil Whirl'  ,'f Oil Whirl'  ,'BW Oil Whirl'  ,
                  ...
                  'i 1BPF'       ,'P 1BPF'       ,'E 1BPF'       ,'f 1BPF'       ,'BW 1BPF'       ,
                  'i 1BPF-1.0'   ,'P 1BPF-1.0'   ,'E 1BPF-1.0'   ,'f 1BPF-1.0'   ,'BW 1BPF-1.0'   ,
                  'i 1BPF+1.0'   ,'P 1BPF+1.0'   ,'E 1BPF+1.0'   ,'f 1BPF+1.0'   ,'BW 1BPF+1.0'   ,
                  'i 2BPF'       ,'P 2BPF'       ,'E 2BPF'       ,'f 2BPF'       ,'BW 2BPF'       ,
                  'i 2BPF-1.0'   ,'P 2BPF-1.0'   ,'E 2BPF-1.0'   ,'f 2BPF-1.0'   ,'BW 2BPF-1.0'   ,
                  'i 2BPF+1.0'   ,'P 2BPF+1.0'   ,'E 2BPF+1.0'   ,'f 2BPF+1.0'   ,'BW 2BPF+1.0'   ,
                  'T. RMS(t)'    ,'T. RMS(f)'                       ]         
    ...
    for medida in range(n_traces):
        ...
        if 24<f[n_maxi_C]<25.5:
            ...
            for counter,k in enumerate(indexes) :   
                #print('mendid',counter,'-----------','k=',k,'left_ips',properties["left_ips"][counter],'right_ips',properties["right_ips"][counter])
                if properties["left_ips"][counter]<k<properties["right_ips"][counter] == False:
                ...
                for w in peaks:
                    if w -delta  <= f[k]/f_1x <= w +delta:
                        ...
                        if piko > df_harm.iloc[medida]['E '+word]:
                            ...
                            df_harm.iloc[medida]['BW '+word] = f[int(properties["widths"][counter]) ]
                                #------AERODYNAMIC FAILURE--------
                if n_blades-delta <= f[k]/f_1x <= n_blades+delta:
                    #print('holaaaaaaaa',f[k],df_harm.iloc[medida]['f 1BPF'])
                    piko = np.sqrt(np.sum( sptrm_C[    int(np.round(properties["left_ips"][counter]) )  : int(np.round(properties["right_ips"][counter] )) +1     ]**2 ))
                    if piko > df_harm.iloc[medida]['E 1BPF']:
                        df_harm.iloc[medida]['i 1BPF']  = k #/ Max_value
                        df_harm.iloc[medida]['P 1BPF']  = sptrm_C[k]
                        df_harm.iloc[medida]['E 1BPF']  = piko #/ Max_value
                        df_harm.iloc[medida]['f 1BPF']  = f[k]
                        df_harm.iloc[medida]['BW 1BPF'] = f[int(properties["widths"][counter]) ]
                if n_blades-1-delta <= f[k]/f_1x <= n_blades-1+delta :
                    piko = np.sqrt(np.sum( sptrm_C[    int(np.round(properties["left_ips"][counter]) )  : int(np.round(properties["right_ips"][counter] )) +1     ]**2 ))
                    if piko > df_harm.iloc[medida]['E 1BPF-1.0']:
                        df_harm.iloc[medida]['i 1BPF-1.0']  = k
                        df_harm.iloc[medida]['P 1BPF-1.0']  = sptrm_C[k]
                        df_harm.iloc[medida]['E 1BPF-1.0']  = piko #/ Max_value
                ...
                if 2*n_blades-delta <= f[k]/f_1x <= 2*n_blades+delta:
                    piko = np.sqrt(np.sum( sptrm_C[    int(np.round(properties["left_ips"][counter]) )  : int(np.round(properties["right_ips"][counter] )) +1     ]**2 ))
                    if piko > df_harm.iloc[medida]['E 2BPF']:
                        df_harm.iloc[medida]['i 2BPF']  = k
                        df_harm.iloc[medida]['P 2BPF']  = sptrm_C[k]
                        df_harm.iloc[medida]['E 2BPF']  = piko #/ Max_value
                        df_harm.iloc[medida]['f 2BPF']  = f[k]
                        df_harm.iloc[medida]['BW 2BPF'] = f[int(properties["widths"][counter]) ]
                    
                if 2*n_blades-1-delta <= f[k]/f_1x <= 2*n_blades-1+delta :
                    piko = np.sqrt(np.sum( sptrm_C[    int(np.round(properties["left_ips"][counter]) )  : int(np.round(properties["right_ips"][counter] )) +1     ]**2 ))
                    if piko > df_harm.iloc[medida]['E 2BPF-1.0']:
                        df_harm.iloc[medida]['i 2BPF-1.0']  = k
                        df_harm.iloc[medida]['P 2BPF-1.0']  = sptrm_C[k]
                        df_harm.iloc[medida]['E 2BPF-1.0']  = piko #/ Max_value
