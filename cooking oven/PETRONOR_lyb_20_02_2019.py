...
def Load_Vibration_Data_Global(parameters):
    ...
    if parameters['Source'] == 'Local Database':   
        ...
        if Found :
            ...
            df_accel,l_devices = Load_Vibration_Data_From_DB(parameters['Path']+'\\'+month+'\\'+day+'\\'+hour,parameters['IdAsset'],parameters['Localizacion'])
            df_speed,df_SPEED  = velocity(df_accel)
...
def Ball_Bearing_Outer_Race_Defects(df_in):
    print('Ball B. O. Race D. Failure')
    n_traces   = df_in.shape[0]
    none_list  = []

    for i in range (n_traces):
        none_list.append('None')
    df_in['Ball B. O. Race D. Failure'] = none_list

    for i in range (n_traces):

        A = (df_in.iloc[i]['RMS BPFO1'] > E1) or (df_in.iloc[i]['RMS BPFO2'] > E1)
        
        b1 = (df_in.iloc[i]['RMS BPFO1'] > E1) and (df_in.iloc[i]['RMS 2*BPFO1'] > E1)
        b2 = (df_in.iloc[i]['RMS BPFO2'] > E1) and (df_in.iloc[i]['RMS 2*BPFO2'] > E1)
        B = b1 or b2
        
        c1 = (df_in.iloc[i]['RMS BPFO1'] > E1) and (df_in.iloc[i]['RMS 2*BPFO1'] > E1) and (df_in.iloc[i]['RMS 3*BPFO1'] > E1)
        c2 = (df_in.iloc[i]['RMS BPFO2'] > E1) and (df_in.iloc[i]['RMS 2*BPFO2'] > E1) and (df_in.iloc[i]['RMS 3*BPFO2'] > E1)
        C = c1 or c2
        
        d1 = (df_in.iloc[i]['RMS BPFO1'] > E1) and (df_in.iloc[i]['RMS 2*BPFO1'] > E1) and (df_in.iloc[i]['RMS 3*BPFO1'] > E1) and (df_in.iloc[i]['RMS 4*BPFO1'] > E1)
        d2 = (df_in.iloc[i]['RMS BPFO2'] > E1) and (df_in.iloc[i]['RMS 2*BPFO2'] > E1) and (df_in.iloc[i]['RMS 3*BPFO2'] > E1) and (df_in.iloc[i]['RMS 4*BPFO2'] > E1)
        D = d1 or d2
        ...
        print(d1,d2)
        if A:
            df_in.loc[df_in.index[i],'Ball B. O. Race D. Failure'] = 'Green'
        if B ^ C:
            df_in.loc[df_in.index[i],'Ball B. O. Race D. Failure'] = 'Yellow'
        if D:
            df_in.loc[df_in.index[i],'Ball B. O. Race D. Failure'] = 'Red'
#        
#        print(D)
#        print('________________',df_in.loc[df_in.index[i],'Pillow block Loseness'])
    return df_in
...
def Ball_Bearing_Inner_Race_Defects(df_in):
    print('Ball B. I. Race D. Failure')
    n_traces   = df_in.shape[0]
    none_list  = []

    for i in range (n_traces):
        none_list.append('None')
    df_in['Ball B. I. Race D. Failure'] = none_list

    for i in range (n_traces):

        A =  (df_in.iloc[i]['RMS BPFI1'] > E1) or (df_in.iloc[i]['RMS BPFI2'] > E1)
        
        b1 = (df_in.iloc[i]['RMS BPFI1'] > E1) and (df_in.iloc[i]['RMS 2*BPFI1'] > E1)
        b2 = (df_in.iloc[i]['RMS BPFI2'] > E1) and (df_in.iloc[i]['RMS 2*BPFI2'] > E1)
        B = b1 or b2
        
        c1 = (df_in.iloc[i]['RMS BPFI1'] > E1) and (df_in.iloc[i]['RMS BPFI1+f'] > E1) and (df_in.iloc[i]['RMS BPFI1-f'] > E1)
        c2 = (df_in.iloc[i]['RMS BPFI2'] > E1) and (df_in.iloc[i]['RMS BPFI2+f'] > E1) and (df_in.iloc[i]['RMS BPFI2-f'] > E1)
        C = c1 or c2
        
        d1 = (df_in.iloc[i]['RMS 2*BPFI1'] > E1) and (df_in.iloc[i]['RMS 2*BPFI1+f'] > E1) and (df_in.iloc[i]['RMS 2*BPFI1-f'] > E1)
        d2 = (df_in.iloc[i]['RMS 2*BPFI2'] > E1) and (df_in.iloc[i]['RMS 2*BPFI2+f'] > E1) and (df_in.iloc[i]['RMS 2*BPFI2-f'] > E1)
        D = d1 or d2

        #print ('Pillow block Loseness',A,B)
        if A:
            df_in.loc[df_in.index[i],'Ball B. I. Race D. Failure'] = 'Green'
        if B ^ C:
            df_in.loc[df_in.index[i],'Ball B. I. Race D. Failure'] = 'Yellow'
        if D:
            df_in.loc[df_in.index[i],'Ball B. I. Race D. Failure'] = 'Red'
#        
#        print(D)
#        print('________________',df_in.loc[df_in.index[i],'Pillow block Loseness'])
    return df_in
...
def Ball_Bearing_Defect(df_in):
    print('Ball B D. Failure')
    n_traces   = df_in.shape[0]
    none_list  = []

    for i in range (n_traces):
        none_list.append('None')
    df_in['Ball B D. Failure'] = none_list

    for i in range (n_traces):

        A =  (df_in.iloc[i]['RMS BSF1'] > E1) or (df_in.iloc[i]['RMS BSF2'] > E1)
        
        b1 = (df_in.iloc[i]['RMS BSF1'] > E1) and (df_in.iloc[i]['RMS 2*BSF1'] > E1)
        b2 = (df_in.iloc[i]['RMS BSF2'] > E1) and (df_in.iloc[i]['RMS 2*BSF2'] > E1)
        B = b1 or b2
        
        c1 = (df_in.iloc[i]['RMS BSF1'] > E1) and (df_in.iloc[i]['RMS BSF1+FTF1'] > E1) and (df_in.iloc[i]['RMS BSF1-FTF1'] > E1)
        c2 = (df_in.iloc[i]['RMS BSF2'] > E1) and (df_in.iloc[i]['RMS BSF2+FTF2'] > E1) and (df_in.iloc[i]['RMS BSF2-FTF2'] > E1)
        C = c1 or c2
        
        d1 = (df_in.iloc[i]['RMS 2*BSF1'] > E1) and (df_in.iloc[i]['RMS 2*BSF1+FTF1'] > E1) and (df_in.iloc[i]['RMS 2*BSF1-FTF1'] > E1)
        d2 = (df_in.iloc[i]['RMS 2*BSF2'] > E1) and (df_in.iloc[i]['RMS 2*BSF2+FTF2'] > E1) and (df_in.iloc[i]['RMS 2*BSF2-FTF2'] > E1)
        D = d1 or d2

        #print ('Pillow block Loseness',A,B)
        if A:
            df_in.loc[df_in.index[i],'Ball B D. Failure'] = 'Green'
        if B ^ C:
            df_in.loc[df_in.index[i],'Ball B D. Failure'] = 'Yellow'
        if D:
            df_in.loc[df_in.index[i],'Ball B D. Failure'] = 'Red'
#        
#        print(D)
#        print('________________',df_in.loc[df_in.index[i],'Pillow block Loseness'])
...
def Ball_Bearing_Cage_Defect(df_in):
    print('Ball B. Cage D. Failure')
    n_traces   = df_in.shape[0]
    none_list  = []

    for i in range (n_traces):
        none_list.append('None')
    df_in['Ball B. Cage D. Failure'] = none_list

    for i in range (n_traces):

        a1 = ( (df_in.iloc[i]['RMS 1.0'] > E1) and (df_in.iloc[i]['RMS FTF1'] > E1) ) and ( df_in.iloc[i]['RMS FTF1'] < df_in.iloc[i]['RMS 1.0'] ) 
        a2 = ( (df_in.iloc[i]['RMS 1.0'] > E1) and (df_in.iloc[i]['RMS FTF2'] > E1) ) and ( df_in.iloc[i]['RMS FTF2'] < df_in.iloc[i]['RMS 1.0']) 
        A = a1 or a2
        
        b1 = ( (df_in.iloc[i]['RMS 1.0'] > E1) and (df_in.iloc[i]['RMS FTF1'] > E1) and (df_in.iloc[i]['RMS 2*FTF1'] > E1) ) and ( df_in.iloc[i]['RMS FTF1'] > df_in.iloc[i]['RMS 2*FTF1'] < df_in.iloc[i]['RMS 1.0'] ) 
        b2 = ( (df_in.iloc[i]['RMS 1.0'] > E1) and (df_in.iloc[i]['RMS FTF2'] > E1) and (df_in.iloc[i]['RMS 2*FTF2'] > E1) ) and ( df_in.iloc[i]['RMS FTF2'] > df_in.iloc[i]['RMS 2*FTF2'] < df_in.iloc[i]['RMS 1.0'] )
        B = b1 or b2
        
        c11 = ( (df_in.iloc[i]['RMS 1.0'] > E1) and (df_in.iloc[i]['RMS FTF1'] > E1) and (df_in.iloc[i]['RMS 2*FTF1'] > E1)  and (df_in.iloc[i]['RMS 3*FTF1'] > E1)  and (df_in.iloc[i]['RMS 4*FTF1'] > E1) ) 
        c12 = ( df_in.iloc[i]['RMS FTF1'] > df_in.iloc[i]['RMS 2*FTF1'] > df_in.iloc[i]['RMS 1.0'] > df_in.iloc[i]['RMS 3*FTF1'] > df_in.iloc[i]['RMS 4*FTF1'])
        c1 = c11 and c12
        
        c21 = ( (df_in.iloc[i]['RMS 1.0'] > E1) and (df_in.iloc[i]['RMS FTF1'] > E1) and (df_in.iloc[i]['RMS 2*FTF1'] > E1)  and (df_in.iloc[i]['RMS 3*FTF1'] > E1)  and (df_in.iloc[i]['RMS 4*FTF1'] > E1) ) 
        c22 = ( df_in.iloc[i]['RMS FTF1'] > df_in.iloc[i]['RMS 2*FTF1'] > df_in.iloc[i]['RMS 1.0'] > df_in.iloc[i]['RMS 3*FTF1'] > df_in.iloc[i]['RMS 4*FTF1'])
        c2 = c21 and c22
        ...
        #print ('Pillow block Loseness',A,B)
        if A:
            df_in.loc[df_in.index[i],'Ball B. Cage D. Failure'] = 'Green'
        if B:
            df_in.loc[df_in.index[i],'Ball B. Cage D. Failure'] = 'Yellow'
        if C:
            df_in.loc[df_in.index[i],'Ball B. Cage D. Failure'] = 'Red'
#        
#        print(D)
#        print('________________',df_in.loc[df_in.index[i],'Pillow block Loseness'])
    return df_in
...
def plot_waterfall2(df_in,df_harm,fs,fmin,fmax):
    ...
    if escala == 'Peak':
        ...
        cte   = 2* np.sqrt(2)               # 2 => hanning , 2 => 1º Z. Niquist
        label = 'RMS'
